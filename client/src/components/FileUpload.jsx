...
export default function FileUpload({ onUpload, multiple = false, endpoint, onImageSelect }) {
	...
	const handleUpload = useCallback(async () => {
		...
		try {
			const { data } = await server.post(endpoint, formData);
	...
	return (
		<VStack spacing={6} w="100%" align="center" justify="center">
			<input type="file" max={10} ref={fileInputRef} onChange={handleFileChange} multiple={multiple} accept="image/*" hidden />

			{multiple && endpoint === "/populate" && (
				<FormControl>
					<FormLabel>Populate a category</FormLabel>
					<Input mt={2} placeholder="Enter a category name" value={category} onChange={handleCategoryChange} bg="black" borderColor="rgba(255, 255, 255, 0.3)" _hover={{ borderColor: "rgba(255, 255, 255, 0.5)" }} />
				</FormControl>
			)}

			{!hasSelectedFile && (
				<MotionButton onClick={handleFileInputClick} leftIcon={<AttachmentIcon />} colorScheme="red" size="lg" variants={buttonVariants} whileHover="hover" whileTap="tap" borderRadius="full" shadow="md" bg="red.500" color="white">
			...
			{multiple && previews.length > 1 && (
				<HStack spacing={2} overflowX="auto" maxW="100%" p={2} mb={2}>
					{previews.map((preview, index) => (
						...
						>
							<Image src={preview} alt={`Thumbnail ${index + 1}`} h="50px" w="50px" objectFit="cover" borderRadius="md" />
							<IconButton
								icon={<CloseIcon />}
								size="xs"
								position="absolute"
